...
public static class KeyValueStoreRlpExtensions
{
    ...
    public static TItem? Get<TItem>(this IReadOnlyKeyValueStore db, long blockNumber, ValueHash256 hash, IRlpStreamDecoder<TItem> decoder,
        ICache<ValueHash256, TItem> cache = null, bool shouldCache = true) where TItem : class
    {
    ...
    public static TItem? Get<TItem>(this IReadOnlyKeyValueStore db, ValueHash256 key, IRlpStreamDecoder<TItem> decoder, ICache<ValueHash256, TItem> cache = null, bool shouldCache = true) where TItem : class
    {
        return Get(db, key, key.Bytes, decoder, cache, shouldCache);
    }

    public static TItem? Get<TItem>(this IReadOnlyKeyValueStore db, long key, IRlpStreamDecoder<TItem>? decoder, ICache<long, TItem>? cache = null, bool shouldCache = true) where TItem : class
    {
